...
KKpipiVersusKKDoubleTag::KKpipiVersusKKDoubleTag(const std::string &name, ISvcLocator *pSvcLocator): Algorithm(name, pSvcLocator) {
  declareProperty("dummy", m_dummy = 0);
}

KKpipiVersusKKDoubleTag::~KKpipiVersusKKDoubleTag() {
}

StatusCode KKpipiVersusKKDoubleTag::initialize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Initializing KKpipi vs KK Double Tagging" << endreq;
  StatusCode status;
  NTuplePtr ntp(ntupleSvc(), "KKPIPI/KKDoubleTag");
  if(ntp) {
    m_tuple = ntp;
  } else {
    m_tuple = ntupleSvc()->book("KKPIPI/KKDoubleTag", CLID_ColumnWiseTuple, "Double tagged D->KKpipi vs D->KK events");
    if(m_tuple) {
      ...
      status = m_tuple->addItem("TagKMinuspx", m_TagKPluspx);
      status = m_tuple->addItem("TagKMinuspy", m_TagKPluspy);
      status = m_tuple->addItem("TagKMinuspz", m_TagKPluspz);
      status = m_tuple->addItem("TagKMinusenergy", m_TagKPlusenergy);
    } else {
      log << MSG::ERROR << "Cannot book NTuple for KKpipi vs KK Double Tags" << endmsg;
...
StatusCode KKpipiVersusKKDoubleTag::execute() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Executing KKpipi vs KK Double Tag Algorithm" << endreq;
  ...
  if(DTTool.findDTag(EvtRecDTag::kD0toKKPiPi, EvtRecDTag::kD0toKK)) {
