...
StatusCode KKpipiVersusKSetaPrimepipipi0DoubleTag::initialize() {
  ...
  if(ntp) {
    ...
    if(m_tuple) {
      ...
      status = m_tuple->addItem("TagHighEEtapx", m_TagHighEPi0px);
      status = m_tuple->addItem("TagHighEEtapy", m_TagHighEPi0py);
      status = m_tuple->addItem("TagHighEEtapz", m_TagHighEPi0pz);
      status = m_tuple->addItem("TagHighEEtaenergy", m_TagHighEPi0energy);
      status = m_tuple->addItem("TagLowEEtapx", m_TagLowEPi0px);
      status = m_tuple->addItem("TagLowEEtapy", m_TagLowEPi0py);
      status = m_tuple->addItem("TagLowEEtapz", m_TagLowEPi0pz);
      status = m_tuple->addItem("TagLowEEtaenergy", m_TagLowEPi0energy);
      status = m_tuple->addItem("TagHighEEtaConstrainedpx", m_TagHighEPi0Constrainedpx);
      status = m_tuple->addItem("TagHighEEtaConstrainedpy", m_TagHighEPi0Constrainedpy);
      status = m_tuple->addItem("TagHighEEtaConstrainedpz", m_TagHighEPi0Constrainedpz);
      status = m_tuple->addItem("TagHighEEtaConstrainedenergy", m_TagHighEPi0Constrainedenergy);
      status = m_tuple->addItem("TagLowEEtaConstrainedpx", m_TagLowEPi0Constrainedpx);
      status = m_tuple->addItem("TagLowEEtaConstrainedpy", m_TagLowEPi0Constrainedpy);
      status = m_tuple->addItem("TagLowEEtanConstrainedpz", m_TagLowEPi0Constrainedpz);
      status = m_tuple->addItem("TagLowEEtaConstrainedenergy", m_TagLowEPi0Constrainedenergy);
      status = m_tuple->addItem("TagEtaChi2Fit", m_Pi0Chi2Fit);
...
StatusCode KKpipiVersusKSetaPrimepipipi0DoubleTag::FillTuple(DTagToolIterator DTTool_Signal_iter, DTagToolIterator DTTool_Tag_iter, DTagTool &DTTool) {
  ...
  m_TagHigheEEtanergy = findEta.GetHighEPhotonP(3);
  ...
  m_TagLowEEtanConstrainedenergy = findEta.GetLowEPhotonPConstrained(3);
