...
// KKpipi
#include "KKpipi/KKpipiVersusKLKKDoubleTag.h"
...
DECLARE_COMPONENT(KKpipiVersusKLKKDoubleTag)

KKpipiVersusKLKKDoubleTag::KKpipiVersusKLKKDoubleTag(const std::string &name, ISvcLocator *pSvcLocator): Algorithm(name, pSvcLocator) {
  declareProperty("dummy", m_dummy = 0);
}

KKpipiVersusKLKKDoubleTag::~KKpipiVersusKLKKDoubleTag() {
}

StatusCode KKpipiVersusKLKKDoubleTag::initialize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Initializing KKpipi vs KLKK Double Tagging" << endreq;
  StatusCode status;
  NTuplePtr ntp(ntupleSvc(), "KKPIPI/KLKKDoubleTag");
  if(ntp) {
    m_tuple = ntp;
  } else {
    m_tuple = ntupleSvc()->book("KKPIPI/KLKKDoubleTag", CLID_ColumnWiseTuple, "Double tagged D->KKpipi vs D->KLKK events");
    if(m_tuple) {
      ...
      log << MSG::ERROR << "Cannot book NTuple for KKpipi vs KLKK Double Tags" << endmsg;
...
StatusCode KKpipiVersusKLKKDoubleTag::execute() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Executing KKpipi vs KLKK Double Tag Algorithm" << endreq;
  DTagTool DTTool;
  DTTool.setPID(true);
...
StatusCode KKpipiVersusKLKKDoubleTag::finalize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Finalizing KKpipi vs KLKK Double Tagging" << endreq;
  return StatusCode::SUCCESS;
}

StatusCode KKpipiVersusKLKKDoubleTag::FillTuple(DTagToolIterator DTTool_Signal_iter, DTagTool &DTTool) {
  ...
  m_TagKPluspx = findKL.GethPlusP(0);
  m_TagKPluspy = findKL.GethPlusP(1);
  m_TagKPluspz = findKL.GethPlusP(2);
  m_TagKPlusenergy = findKL.GethPlusP(3);
  m_TagKMinuspx = findKL.GethMinusP(0);
  m_TagKMinuspy = findKL.GethMinusP(1);
  ...
  m_TagKPluspxKalmanFit = findKL.GethPlusPKalmanFit(0);
  m_TagKPluspyKalmanFit = findKL.GethPlusPKalmanFit(1);
  m_TagKPluspzKalmanFit = findKL.GethPlusPKalmanFit(2);
  m_TagKPlusenergyKalmanFit = findKL.GethPlusPKalmanFit(3);
  m_TagKMinuspxKalmanFit = findKL.GethMinusPKalmanFit(0);
  m_TagKMinuspyKalmanFit = findKL.GethMinusPKalmanFit(1);
  m_TagKMinuspzKalmanFit = findKL.GethMinusPKalmanFit(2);
  m_TagKMinusenergyKalmanFit = findKL.GethMinusPKalmanFit(3);
