...
KKpipi::KKpipi(const std::string& name, ISvcLocator* pSvcLocator): Algorithm(name, pSvcLocator) {
  ...
  declareProperty("KKpipiVersusKLpi0DoubleTag", m_recKLpi0Tag = true);
  declareProperty("KKpipiVersusKLpi0pi0DoubleTag", m_recKLpi0pi0Tag = true);
  declareProperty("KKpipiVersusKLpipipi0DoubleTag", m_recKLpipipi0Tag = true);
  declareProperty("KKpipiVersusKLpipiDoubleTag", m_recKLpipiTag = true);
  declareProperty("KKpipiVersusKLKKDoubleTag", m_recKLKKTag = true);
  declareProperty("KKpipiVersusKeNuDoubleTag", m_recKeNuTag = true);
}
...
StatusCode KKpipi::initialize() {
  ...
  if(m_recKKTag) {
    ...
    sc = createSubAlgorithm("KKpipiVersusKKPartRecoDoubleTag", "KKpipiVersusKKPartRecoDoubleTag", m_KKPartRecoTag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKKPartRecoDoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  ...
  if(m_recKSpipiTag) {
    ...
    sc = createSubAlgorithm("KKpipiVersusKSpipiPartRecoDoubleTag", "KKpipiVersusKSpipiPartRecoDoubleTag", m_KSpipiPartRecoTag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKSpipiPartRecoDoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  ...
  if(m_recKpipipiTag) {
    sc = createSubAlgorithm("KKpipiVersusKpipipiDoubleTag", "KKpipiVersusKpipipiDoubleTag", m_KpipipiTag);
    if(sc.isFailure()) {
  ...
  if(m_recKLpi0Tag) {
    sc = createSubAlgorithm("KKpipiVersusKLpi0DoubleTag", "KKpipiVersusKLpi0DoubleTag", m_KLpi0Tag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKLpi0DoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  }
  if(m_recKLpi0pi0Tag) {
    sc = createSubAlgorithm("KKpipiVersusKLpi0pi0DoubleTag", "KKpipiVersusKLpi0pi0DoubleTag", m_KLpi0pi0Tag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKLpi0pi0DoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  }
  if(m_recKLpipipi0Tag) {
    sc = createSubAlgorithm("KKpipiVersusKLpipipi0DoubleTag", "KKpipiVersusKLpipipi0DoubleTag", m_KLpipipi0Tag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKLpipipi0DoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  }
  if(m_recKLpipiTag) {
    sc = createSubAlgorithm("KKpipiVersusKLpipiDoubleTag", "KKpipiVersusKLpipiDoubleTag", m_KLpipiTag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKLpipiDoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  }
  if(m_recKLKKTag) {
    sc = createSubAlgorithm("KKpipiVersusKLKKDoubleTag", "KKpipiVersusKLKKDoubleTag", m_KLKKTag);
    if(sc.isFailure()) {
      log << MSG::ERROR << "Error while creating KKpipiVersusKLKKDoubleTag" << endreq;
      return StatusCode::FAILURE;
    }
  }
  if(m_recKeNuTag) {
