...
KKpipiVersusKpipipiDoubleTag::KKpipiVersusKpipipiDoubleTag(const std::string &name, ISvcLocator *pSvcLocator): Algorithm(name, pSvcLocator) {
  declareProperty("dummy", m_dummy = 0);
}

KKpipiVersusKpipipiDoubleTag::~KKpipiVersusKpipipiDoubleTag() {
}

StatusCode KKpipiVersusKpipipiDoubleTag::initialize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Initializing KKpipi vs Kpipipi Double Tagging" << endreq;
  StatusCode status;
  NTuplePtr ntp(ntupleSvc(), "KKPIPI/KpipipiDoubleTag");
  if(ntp) {
    m_tuple = ntp;
  } else {
    m_tuple = ntupleSvc()->book("KKPIPI/KpipipiDoubleTag", CLID_ColumnWiseTuple, "Double tagged D->KKpipi vs D->Kpipipi events");
    if(m_tuple) {
      ...
      status = m_tuple->addItem("Tag12KSMassFit", m_Tag12KSMassFit);
      ...
      status = m_tuple->addItem("Tag13KSMassFit", m_Tag13KSMassFit);
      ...
      log << MSG::ERROR << "Cannot book NTuple for KKpipi vs Kpipipi Double Tags" << endmsg;
...
StatusCode KKpipiVersusKpipipiDoubleTag::execute() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Executing KKpipi vs Kpipipi Double Tag Algorithm" << endreq;
  ...
  if(DTTool.findDTag(EvtRecDTag::kD0toKKPiPi, EvtRecDTag::kD0toKPiPiPi)) {
    ...
    if(FillTupleStatus != StatusCode::SUCCESS) {
      log << MSG::FATAL << "Assigning Kpipipi tuple info failed" << endreq;
      return StatusCode::FAILURE;
    }
    m_tuple->write();
  }
...
StatusCode KKpipiVersusKpipipiDoubleTag::finalize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Finalizing KKpipi vs Kpipipi Double Tagging" << endreq;
  return StatusCode::SUCCESS;
}

StatusCode KKpipiVersusKpipipiDoubleTag::FillTuple(DTagToolIterator DTTool_Signal_iter, DTagToolIterator DTTool_Tag_iter, DTagTool &DTTool) {
  if(m_RunNumber < 0) {
  ...
  m_Tag12KSMassFit = findKpipipiTagInfo.GetKSMassFit12();
  ...
  m_Tag13KSMassFit = findKpipipiTagInfo.GetKSMassFit13();
  if(m_RunNumber < 0) {
