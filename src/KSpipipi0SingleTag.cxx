...
DECLARE_COMPONENT(KSpipipi0SingleTag)

KSpipipi0SingleTag::KSpipipi0SingleTag(const std::string &name, ISvcLocator *pSvcLocator): Algorithm(name, pSvcLocator) {
  declareProperty("dummy", m_dummy = 0);
}

KSpipipi0SingleTag::~KSpipipi0SingleTag() {
}

StatusCode KSpipipi0SingleTag::initialize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Initializing KSpipipi0 Single Tagging" << endreq;
  StatusCode status;
  NTuplePtr ntp(ntupleSvc(), "KKPIPI/KSpipipi0SingleTag");
  if(ntp) {
    m_tuple = ntp;
  } else {
    m_tuple = ntupleSvc()->book("KKPIPI/KSpipipi0SingleTag", CLID_ColumnWiseTuple, "Single tagged D->KSpipipi0 events");
    if(m_tuple) {
      ...
      status = m_tuple->addItem("PiPlusMotherTrueID", m_PiPlusMotherTrueID);
      status = m_tuple->addItem("PiMinusMotherTrueID", m_PiMinusMotherTrueID);
    } else {
...
StatusCode KSpipipi0SingleTag::finalize() {
  MsgStream log(msgSvc(), name());
  log << MSG::INFO << "Finalizing KSPiPiPi0 Single Tagging" << endreq;
  return StatusCode::SUCCESS;
}

StatusCode KSpipipi0SingleTag::FillTuple(DTagToolIterator DTTool_iter, DTagTool &DTTool) {
  if(m_RunNumber < 0) {
  ...
  if(m_RunNumber < 0) {
    ...
    m_HighEPi0PhotonTrueID = ReconstructedPID[4];
    m_LowEPi0PhotonTrueID = ReconstructedPID[5];
    m_PiPlusMotherTrueID = PID_Truth.GetTrueMotherID(DaughterTrackIDs[0], true);
    m_PiMinusMotherTrueID = PID_Truth.GetTrueMotherID(DaughterTrackIDs[1], true);
    ...
    m_HighEPi0PhotonMotherTrueID = PID_Truth.GetTrueMotherID(PhotonPairTrackID[0].first, false);
    m_LowEPi0PhotonMotherTrueID = PID_Truth.GetTrueMotherID(PhotonPairTrackID[0].second, false);
  }
  return StatusCode::SUCCESS;
}
