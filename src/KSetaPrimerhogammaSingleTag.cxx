...
StatusCode KSetaPrimerhogammaSingleTag::initialize() {
  ...
  if(ntp) {
    ...
    if(m_tuple) {
      ...
      status = m_tuple->addItem("PhotonTrueID", m_PhotonTrueID);
      ...
      status = m_tuple->addItem("PhotonMotherTrueID", m_PhotonMotherTrueID);
...
StatusCode KSetaPrimerhogammaSingleTag::FillTuple(DTagToolIterator DTTool_iter, DTagTool &DTTool) {
  ...
  if(m_RunNumber < 0) {
    ...
    m_PhotonTrueID = ReconstructedPID[4];
    ...
    m_PhotonMotherTrueID = PID_Truth.GetTrueMotherID(DaughterTrackIDs[4], false);
  }
  return StatusCode::SUCCESS;
}
