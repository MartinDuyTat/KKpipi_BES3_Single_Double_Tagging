...
StatusCode KKpipiVersusKSetaDoubleTag::execute() {
  ...
  if(DTTool.findDTag(EvtRecDTag::kD0toKKPiPi, EvtRecDTag::kD0toKsEta)) {
    ...
    if(FillTupleStatus != StatusCode::SUCCESS) {
      log << MSG::FATAL << "Assigning KSeta tuple info failed" << endreq;
      return StatusCode::FAILURE;
    }
    m_tuple->write();
  }
...
StatusCode KKpipiVersusKSetaDoubleTag::FillTuple(DTagToolIterator DTTool_Signal_iter, DTagToolIterator DTTool_Tag_iter, DTagTool &DTTool) {
  ...
  if(m_RunNumber < 0) {
    ...
    m_TagHighEEtaPhotonTrueID = ReconstructedPID[2];
    m_TagLowEEtaPhotonTrueID = ReconstructedPID[3];
    ...
    m_TagHighEEtaPhotonMotherTrueID = PID_Truth.GetTrueMotherID(DaughterTrackIDs[2], false);
    m_TagLowEEtaPhotonMotherTrueID = PID_Truth.GetTrueMotherID(DaughterTrackIDs[3], false);
  }
  return StatusCode::SUCCESS;
}
