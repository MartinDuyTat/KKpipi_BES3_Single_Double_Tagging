...
/**
 * KSpipipi0SingleTag is a class for a BOSS algorithm
 * It runs over \f$D^0\bar{D^0}\f$ data and saves all events with a single \f$D\to K_S\pi^+\pi^-\pi^0f$ tag
 */

#ifndef KSPIPIPI0SINGLETAG
#define KSPIPIPI0SINGLETAG
...
class KSpipipi0SingleTag: public Algorithm {
  public: 
    /**
     * Default constructor for an algorithm where all necessary properties are declared
     */
    KSpipipi0SingleTag(const std::string& name, ISvcLocator* pSvcLocator);
    /**
     * Trivial destructor
     */
    ~KSpipipi0SingleTag();
