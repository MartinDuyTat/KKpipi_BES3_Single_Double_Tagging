...
/**
 * KKpipiVersusKLKKDoubleTag is a class for a BOSS algorithm
 * It runs over \f$D^0\bar{D^0}\f$ data and saves all events with a double \f$D\to K^+K^-\pi^+\pi^-\f$ vs \f$K_LKK\f$ double tags
...
#ifndef KKPIPIVERSUSKLKKDOUBLETAG
#define KKPIPIVERSUSKLKKDOUBLETAG
...
class KKpipiVersusKLKKDoubleTag: public Algorithm {
  public: 
    /**
     * Default constructor for an algorithm where all necessary properties are declared
     */
    KKpipiVersusKLKKDoubleTag(const std::string& name, ISvcLocator* pSvcLocator);
    /**
     * Trivial destructor
     */
    ~KKpipiVersusKLKKDoubleTag();
    /**
  ...
  private:
    ...
    /**
     * Tag \f$K^+\f$ momentum along \f$x\f$ after Kalman fit
     */
    NTuple::Item<double> m_TagKPluspxKalmanFit;
    /**
     * Tag \f$K^+\f$ momentum along \f$y\f$ after Kalman fit
     */
    NTuple::Item<double> m_TagKPluspyKalmanFit;
    /**
     * Tag \f$K^+\f$ momentum along \f$z\f$ after Kalman fit
     */
    NTuple::Item<double> m_TagKPluspzKalmanFit;
    /**
     * Tag \f$K^=\f$ energy after Kalman fit
     */
