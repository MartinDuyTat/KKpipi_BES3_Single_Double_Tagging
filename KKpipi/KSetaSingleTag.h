...
/**
 * KSetaSingleTag is a class for a BOSS algorithm
 * It runs over \f$D^0\bar{D^0}\f$ data and saves all events with a single \f$D\to K_S^0\eta\f$ tag
 */

#ifndef KSETASINGLETAG
#define KSETASINGLETAG
...
class KSetaSingleTag: public Algorithm {
  public: 
    /**
     * Default constructor for an algorithm where all necessary properties are declared
     */
    KSetaSingleTag(const std::string& name, ISvcLocator* pSvcLocator);
    /**
     * Trivial destructor
     */
    ~KSetaSingleTag();
